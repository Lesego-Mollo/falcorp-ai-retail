...
function Cart({ cartItems, onUpdateQuantity, onRemoveItem }: CartProps) {
  ...
  const parsePrice = (p: string) =>
    Number(p.replace(/[^\d.,]/g, "").replace(",", ".")) || 0;
  ...
  const handleCheckout = () => {
    ...
    navigate("/checkout", {
      state: {
        items: cartItems,
        subtotal,
        delivery: cartItems.length > 0 ? DELIVERY_FEE : 0,
        total,
      },
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex flex-col items-center p-4 font-sans">
      ...
      <div className="relative z-10 w-full max-w-4xl mt-8">
        ...
        <div className="glass-morphism rounded-3xl shadow-glass border border-white/30 overflow-hidden">
          {/* Title area */}
          <div className="bg-gradient-to-r from-primary-600 via-primary-500 to-accent-600 text-white py-6 px-6 relative">
              ...
              <>
                ...
                <div className="bg-gradient-to-r from-primary-50 to-accent-50 rounded-2xl p-6 border border-primary-100">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-lg font-semibold text-gray-800">
                      {cartItems.length > 0 ? "R50.00" : "R0.00"}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full bg-gradient-to-r from-primary-500 to-accent-600 text-white py-4 rounded-xl font-bold text-lg shadow-glow hover:shadow-glow-lg transition-all duration-300 ${
                      cartItems.length === 0 ? "opacity-50 cursor-not-allowed" : "hover:scale-105"
                    }`}
                  >
                    Proceed to Checkout
                  </button>
